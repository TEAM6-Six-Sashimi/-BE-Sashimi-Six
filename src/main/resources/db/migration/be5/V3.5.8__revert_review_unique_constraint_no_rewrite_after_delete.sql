...
-- 3. V3.5.5 정책 하에서는 DELETED 리뷰끼리 unique 제약이 없어 같은 (user_id, course_id)에
...
--    이 인덱스가 user_id로 시작하므로, 아래에서 idx_reviews_user_id를 지울 때
--    FK 제약이 이 인덱스를 대신 사용하게 되어 삭제가 가능해짐
ALTER TABLE reviews
    ADD UNIQUE INDEX uq_review_user_course (user_id, course_id);

-- 5. V3.5.5에서 추가한 FK 지원용 인덱스 삭제 (더 이상 필요 없음, 위 unique 제약이 대체)
SET @idx_exists = (
