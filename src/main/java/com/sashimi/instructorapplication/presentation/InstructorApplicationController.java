...
public class InstructorApplicationController {
    ...
    private final InstructorApplicationQueryUseCase instructorApplicationQueryUseCase;
    ...
    public ResponseEntity<Void> applyInstructor(
        ...
        instructorApplicationCommandUseCase.applyInstructor(
    ...
    @ApiResponses({
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "204", description = "승인 완료"),
    ...
    public ResponseEntity<Void> approveInstructor(@PathVariable Long applicationId) {
        instructorApplicationCommandUseCase.approveInstructor(applicationId);
        return ResponseEntity.noContent().build();
    }

    @Operation(summary = "강사 신청 반려 [ADMIN 전용]", description = "관리자만 강사 신청을 반려할 수 있습니다. ROLE_ADMIN 권한 필요.")
    @ApiResponses({
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "204", description = "반려 완료"),
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "400", description = "처리할 수 없는 신청 상태"),
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "401", description = "인증 실패"),
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "403", description = "권한 없음 (ROLE_ADMIN 아닌 경우)"),
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "404", description = "신청을 찾을 수 없음"),
    ...
    public ResponseEntity<Void> rejectInstructor(
        ...
        instructorApplicationCommandUseCase.rejectInstructor(applicationId, request.rejectionCategory(), request.rejectionReason());
        return ResponseEntity.noContent().build();
    }
    ...
    public ResponseEntity<List<InstructorApplicationListResponse>> getPendingInstructorApplications() {
        return ResponseEntity.ok(instructorApplicationQueryUseCase.getPendingInstructorApplications());
    }
    ...
    public ResponseEntity<InstructorApplicationDetailResponse> getInstructorApplicationDetail(
            @PathVariable Long applicationId) {
        return ResponseEntity.ok(instructorApplicationQueryUseCase.getInstructorApplicationDetail(applicationId));
    }
    ...
    public ResponseEntity<List<RejectedApplicationListResponse>> getRejectedInstructorApplications() {
        return ResponseEntity.ok(instructorApplicationQueryUseCase.getRejectedInstructorApplications());
    }
}
